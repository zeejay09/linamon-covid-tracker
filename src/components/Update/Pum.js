...
export default class Pum extends Component {
    ...
    componentDidMount = () => {
        ...
        axios.get('/view/pum/' + this.props.match.params.id, {headers:{Authorization: authToken}}).then(
            response => {
    ...
    render() {
        ...
        <Formik
            ...
            onSubmit={(values) => {
                ...
                console.log(values);
                axios.put('/edit/pum/' + this.state.userId, values, {headers:{Authorization: authToken}}).then(
                    res => {
                        if (res.status === 200) {
